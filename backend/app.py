...
class IngestData(Resource):
    ...
    def get(self):
        ...
        return Response(json.dumps({"task_id": profiling_chord.id, "type": "Ingestion"}), mimetype='application/json', status=202)
...
@api.doc(description="Runs Valentine profiling between the given table and all other ingested tables, used for finding columns that are related.")
@api.doc(params={
    'bucket':  {'description': 'Path to the S3 bucket where the table resides', 'in': 'query', 'type': 'string', 'required': 'true', 'default': 'data'},
...
class ProfileValentine(Resource):
    ...
    def get(self):
        ...
        return Response(json.dumps({"task_id": task.id, "type": "Valentine Profiling"}), mimetype='application/json', status=202)
...
class AddTable(Resource):
    ...
    def get(self):
        ...
        return Response(json.dumps({"task_id": task.id, "type": "Single Ingestion"}), mimetype='application/json', status=202)
...
@api.doc(params={
    'source_table_path':  {'description': 'Path to the source table', 'in': 'query', 'type': 'string', 'required': 'true'},
    'target_table_path':  {'description': 'Path to the target table', 'in': 'query', 'type': 'string', 'required': 'true'}
})
class GetRelatedNodes(Resource):
    @api.response(200, 'Success', api.model("RelatedTables", {"RelatedTables": fields.List(fields.Nested(RelatedTableModel))}))
    ...
    def get(self):
        ...
        if not from_table or not to_table:
            return Response("Please provide both a source table path and a target table path as query parameters", status=400)
...
class GetJoinable(Resource):
    @api.response(200, 'Success', api.model("JoinableTables", {"JoinableTables": fields.List(fields.Nested(JoinableTableModel))}))  # TODO: specify return model
    @api.response(400, 'Missing table paths query parameters')
    @api.response(404, 'Table does not exist')
    def get(self):
        args = request.args
        ...
        return Response(json.dumps({"JoinableTables": discovery.queries.get_joinable(table_path)}), mimetype='application/json', status=200)
